...
def process_embeddings(df, col_name):
    ...
    embeddings_df = pd.DataFrame(
        df[col_name].to_list(),
        columns=[
            f"text_{i + 1}" for i in range(df[col_name].str.len().max())
        ],
    )
...
def rename_image_embeddings(df):
    ...
    """
    df.columns = [
        f"image_{int(col)}" if col.isdigit() else col for col in df.columns
    ]
...
def train_test_split_and_feature_extraction(
    ...
    text_columns = [
        col for col in train_df.columns if col.startswith("text_")
    ]
